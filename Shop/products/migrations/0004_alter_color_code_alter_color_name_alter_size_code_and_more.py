...
def fill_unique_codes(apps, schema_editor):
    ...
    existing = set(
        Color.objects.exclude(code__isnull=True)
        .exclude(code="")
    ...
    for c in Color.objects.all().order_by("id"):
        ...
        if not val or val in existing:
            ...
            while (
                ...
                or Color.objects.exclude(pk=c.pk).filter(code=candidate).exists()
            ):
                i += 1
                candidate = f"{base}{i}"
            c.code = candidate
    ...
    existing = set(
        Size.objects.exclude(code__isnull=True)
        .exclude(code="")
        .values_list("code", flat=True)
    )
    for s in Size.objects.all().order_by("id"):
        ...
        if not val or val in existing:
            ...
            while (
                candidate in existing
                or Size.objects.exclude(pk=s.pk).filter(code=candidate).exists()
            ):
                i += 1
                candidate = f"{base}{i}"
            s.code = candidate
